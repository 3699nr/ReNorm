...
class MSMT17(ImageDataset):
    ...
    def __init__(self, root='datasets', **kwargs):
        self.dataset_dir = osp.join(root, "MSMT17")
        ...
        exdir = self.dataset_dir
        train = self.process_dir(osp.join(exdir, 'list_train.txt'), osp.join(exdir, 'train'), relabel = True) ## 10.27换为v1版本；绝对路径
        val = self.process_dir(osp.join(exdir, 'list_val.txt'), osp.join(exdir, 'train'), relabel = True)
        train = train + val   

        query = self.process_dir(osp.join(exdir, 'list_query.txt'), osp.join(exdir, 'test'), relabel= False)
        gallery = self.process_dir(osp.join(exdir, 'list_gallery.txt'), osp.join(exdir, 'test'), relabel=False)
    ...
    def process_dir(self, list_file, subdir, pattern=re.compile(r'([-\d]+)_c([-\d]+)_([-\d]+)'), relabel=False):
        ...
        for line in lines:
            line = line.strip()
            fname = line.split(' ')[0]
            camid = int(fname.split('/')[1].split('_')[2])
            pid = int(fname.split('/')[0])
            if relabel:
                pid = pid2label[pid]
                ret.append((osp.join(subdir, fname), self.dataset_name + "_" + str(pid), self.dataset_name + "_" + str(camid)))
            else:
                ret.append((osp.join(subdir, fname), pid, camid))
        return ret
