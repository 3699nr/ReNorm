...
class MSMT17All(ImageDataset):
    ...
    dataset_url = None
    dataset_name = 'MSMT17All'

    def __init__(self, root='datasets', **kwargs):
        # root='/data/pengyi/datasets/reid_data/msmt17v2/'
        
        self.dataset_dir = os.path.join(root, 'MSMT17')
        ...
        for img_path, pid, camid in query:
            query_tmp.append((img_path, self.dataset_name + "_" + str(pid+num_train_pids), self.dataset_name + "_" + str(camid))) 
        del query
        query = query_tmp

        gallery_temp = []
        for img_path, pid, camid in gallery:
            gallery_temp.append((img_path, self.dataset_name + "_" + str(pid+num_train_pids), self.dataset_name + "_" + str(camid)))
        del gallery
        ...
        ### new add 1.13 all datasets
        train = train + val + query + gallery
        super(MSMT17All, self).__init__(train, [], [], **kwargs)  
    ...
    def process_dir(self, dir_path, list_path, is_train=True):
        ...
        for img_idx, img_info in enumerate(lines):
            ...
            data.append((img_path, pid, camid))#路径 MSMT17ALL_1040 MSMT17ALL_6
