...
class GRID(ImageDataset):
    ...
    def __init__(self, root, split_id = 0, **kwargs):
        self.root = root
        self.dataset_dir = "datasets/GRID"
        ...
        ## 从json文件选query, 125个id,125张图片, 其余图片都是gallery
        # self.train_1 = []
        self.query = []
        self.query_name = []
        self.gallery = []
        ...
        for item in query:
            self.query_name.append(item[0].split('/')[-1])
            
            q_ids = int(item[0].split('/')[-1].split('_')[0])
            if q_ids not in self.query_ids:
                self.query_ids.append(q_ids)

            item[0] = os.path.join(self.root, item[0])
            self.query.append(tuple(item))
        ...
        for item in gallery:
            ...
            if g_ids == 0:
                self.zero_id += 1
            elif g_ids not in self.gallery_ids:
                self.gallery_ids.append(g_ids)
            
            item[0] = os.path.join(self.root, item[0])
            self.gallery.append(tuple(item))

        probe_list = os.listdir(self.probe_path)
        for p in probe_list:
            pid = int(p.split('_')[0])
            camid = int(p.split('_')[1]) - 1
            if p not in self.query_name:
                if pid not in self.gallery_ids:
                    self.gallery_ids.append(pid)
                self.gallery.append((os.path.join(self.probe_path, p), pid, camid))
        ...
        # print("  gallery  | {:5d} | {:8d}"
        #       .format(len(self.gallery_ids) + self.zero_id, len(self.gallery)))
        
        train = []
        query, gallery = self.query, self.gallery
