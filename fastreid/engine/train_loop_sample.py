...
class SimpleTrainer(TrainerBase):
    ...
    def __init__(self, cfg,  model, data_loader_list, optimizer):
        ...
        self.num_per_datasets = []
        for i in range(len(data_loader_list)):
            self.num_per_datasets.append(len(data_loader_list[i].dataset))
        self.total_imgs = sum(self.num_per_datasets)
        self.total_iters = self.total_imgs//data_loader_list[0]._index_sampler.batch_size
        self.batchsize = data_loader_list[0]._index_sampler.batch_size
    ...
    def run_step(self):
        ...
        if len(self.pickup_list) == 0:
            self.pickup_list = list(range(0, self.total_iters))
            np.random.shuffle(self.pickup_list)
        picked = self.pickup_list.pop(0)
        for i in range(len(self.num_per_datasets)):
            picked = picked - self.num_per_datasets[i]//self.batchsize
            if picked <=0:
                cur_idx = i
                break
